...
- This project uses **Clerk** for authentication, integrated via `convex/auth.config.ts` and `ConvexProviderWithClerk`.
- `convex/auth.config.ts` configures JWT validation with the Clerk issuer domain:
```typescript
...
- On the client, use `ConvexProviderWithClerk` from `convex/react-clerk` with Clerk's `useAuth` hook:
```tsx
...
```
- Use `useConvexAuth()` from `convex/react` (not Clerk's `useAuth`) to check if the user is authenticated with Convex.
- For SSR in TanStack Start, use `getAuth()` from `@clerk/tanstack-react-start/server` in a server function and set the token via `convexQueryClient.serverHttpClient?.setAuth(token)`.
