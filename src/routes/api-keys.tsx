...
function APIKeys() {
  ...
  const { data: keys, isLoading: isLoadingKeys, error: keysError } = useQuery({
    queryKey: ['api-keys'],
    queryFn: async () => {
    ...
    >
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          {isLoadingKeys ? (
            <div className="flex items-center justify-center gap-2 p-8 text-[var(--text-secondary)]">
              <Loader2 size={18} className="animate-spin" />
              <span>Loading API keys...</span>
            </div>
          ) : keysError ? (
            <div className="text-center p-8 text-[var(--error)] text-sm">
              Failed to load keys: {keysError.message}
            </div>
          ) : !keys || keys.length === 0 ? (
            ...
            keys.map((apiKey) => {
              const displayValue = apiKey.start
                ...
                <div key={apiKey.id} className="card rounded-xl p-6">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex-1 relative">
                      <div className="flex items-center h-12 px-4 pr-16 font-mono text-sm bg-[var(--bg-input)] border border-[var(--border-color)] rounded-lg overflow-hidden">
        ...
        <div className="card rounded-xl p-6 bg-[var(--bg-input)] border-dashed">
          ...
          <ul className="space-y-2 text-sm text-[var(--text-secondary)]">
            <li>• Keep your API keys secure and never share them in public repositories</li>
            ...
            <li>• Use separate keys for different environments (development, staging, production)</li>
          </ul>
        </div>
      </div>
    </DashboardLayout>
